"use client";

import { useTranslations } from "next-intl";
import { Link } from "@/i18n/navigation";
import { Icons } from "@/components/icons";
import { ClientYear } from "@/components/client-year";

export function Footer() {
  const t = useTranslations("Inquiry");

  const links = [
    { label: t("footer.links.0.label"), href: t("footer.links.0.href") },
    { label: t("footer.links.1.label"), href: t("footer.links.1.href") },
    { label: t("footer.links.2.label"), href: t("footer.links.2.href") },
    { label: t("footer.links.3.label"), href: t("footer.links.3.href") },
    { label: t("footer.links.4.label"), href: t("footer.links.4.href") },
  ];

  return (
    <footer className="bg-secondary/50">
      <div className="mx-auto max-w-6xl px-6 py-10 md:py-20">
        {/* 3-Column Grid */}
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3 md:gap-8">
          {/* Column 1: Brand */}
          <div>
            <div className="mb-3 flex items-center gap-3 md:mb-5">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 100 100"
                className="h-5 w-5 text-charcoal"
                fill="none"
                stroke="currentColor"
                strokeWidth="2.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
                <path d="M 28 18 C 44 18, 46 40, 50 58 C 55 76, 84 72, 80 42 L 68 50" />
                <path d="M 72 18 C 56 18, 54 40, 50 58 C 45 76, 16 72, 20 42 L 32 50" />
                <line x1="12" y1="85" x2="88" y2="85" />
              </svg>
              <span className="font-serif-display text-lg text-charcoal">
                De novo
              </span>
            </div>
            <p className="mb-3 max-w-xs text-sm leading-relaxed text-muted-foreground md:mb-4">
              {t("footer.brandDescription")}
            </p>
            <div className="flex items-center gap-4">
              <a
                href={t("footer.instagramUrl")}
                target="_blank"
                rel="noopener noreferrer"
                className="text-muted-foreground transition-colors hover:text-charcoal"
                aria-label={t("footer.followInstagram")}
              >
                <Icons.instagram />
              </a>
              <a
                href={t("footer.facebookUrl")}
                target="_blank"
                rel="noopener noreferrer"
                className="text-muted-foreground transition-colors hover:text-charcoal"
                aria-label={t("footer.followFacebook")}
              >
                <Icons.facebook />
              </a>
              <a
                href="https://wa.me/967775228246"
                target="_blank"
                rel="noopener noreferrer"
                className="text-muted-foreground transition-colors hover:text-charcoal"
                aria-label={t("footer.followWhatsapp")}
              >
                <svg
                  className="h-5 w-5"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Column 2: Quick Links */}
          <div>
            <h4 className="mb-3 text-xs font-semibold uppercase tracking-widest text-charcoal md:mb-5">
              {t("footer.quickLinks")}
            </h4>
            <nav className="grid grid-cols-2 gap-x-4 gap-y-1.5 md:grid-cols-1 md:gap-2">
              {links.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="py-1 text-sm text-muted-foreground transition-colors hover:text-charcoal"
                >
                  {link.label}
                </Link>
              ))}
            </nav>
          </div>

          {/* Column 3: Contact Info */}
          <div>
            <h4 className="mb-3 text-xs font-semibold uppercase tracking-widest text-charcoal md:mb-5">
              {t("footer.contactTitle")}
            </h4>
            <ul className="space-y-3 md:space-y-4">
              <li className="flex gap-3">
                <svg
                  className="mt-0.5 h-4 w-4 shrink-0 text-sage"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"
                  />
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z"
                  />
                </svg>
                <div>
                  <p className="hidden text-xs font-semibold uppercase tracking-wider text-charcoal md:block">
                    {t("footer.addressLabel")}
                  </p>
                  <p className="text-sm leading-relaxed text-muted-foreground md:mt-0.5">
                    {t("footer.address")}
                  </p>
                </div>
              </li>
              <li className="flex gap-3">
                <svg
                  className="mt-0.5 h-4 w-4 shrink-0 text-sage"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z"
                  />
                </svg>
                <div>
                  <p className="hidden text-xs font-semibold uppercase tracking-wider text-charcoal md:block">
                    {t("footer.phoneLabel")}
                  </p>
                  <a
                    href={`tel:${t("footer.phone")}`}
                    className="text-sm text-muted-foreground transition-colors hover:text-charcoal md:mt-0.5"
                  >
                    {t("footer.phone")}
                  </a>
                </div>
              </li>
              <li className="flex gap-3">
                <svg
                  className="mt-0.5 h-4 w-4 shrink-0 text-sage"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75"
                  />
                </svg>
                <div>
                  <p className="hidden text-xs font-semibold uppercase tracking-wider text-charcoal md:block">
                    {t("footer.emailLabel")}
                  </p>
                  <a
                    href={`mailto:${t("footer.email")}`}
                    className="text-sm text-muted-foreground transition-colors hover:text-charcoal md:mt-0.5"
                  >
                    {t("footer.email")}
                  </a>
                </div>
              </li>
              <li className="flex gap-3">
                <svg
                  className="mt-0.5 h-4 w-4 shrink-0 text-sage"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                >
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                <div>
                  <p className="hidden text-xs font-semibold uppercase tracking-wider text-charcoal md:block">
                    {t("footer.whatsappLabel")}
                  </p>
                  <a
                    href="https://wa.me/967775228246"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sm text-muted-foreground transition-colors hover:text-charcoal md:mt-0.5"
                  >
                    +967 775228246
                  </a>
                </div>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom Utility Bar */}
        <div className="mt-6 border-t border-champagne/30 pt-6">
          <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
            <p className="text-xs leading-relaxed text-charcoal/60 sm:text-start">
              &copy; <ClientYear /> {t("footer.copyright")}
            </p>
            <p className="text-xs text-charcoal/50">
              {t("footer.businessId")}
              <span className="ms-2" aria-hidden="true">
                |
              </span>
              <span className="ms-2">{t("footer.licensedInsured")}</span>
            </p>
          </div>
        </div>
      </div>
    </footer>
  );
}
